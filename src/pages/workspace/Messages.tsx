...
import { Send, MessageSquarePlus, X, ChevronDown, ArrowLeft } from 'lucide-react';
    ...
    <div className="min-h-screen bg-slate-50 pb-8">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pt-8">
        <div className="flex items-center justify-between mb-6">
          ...
          <button onClick={() => { setCompose(true); setActiveUserId(null); setMobileView('thread'); }} className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-peach-500 text-sm font-bold text-white hover:bg-peach-600 transition-colors">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 h-[calc(100vh-200px)] min-h-[520px]">
          ...
          <div className={cn("bg-white rounded-2xl border border-slate-100 shadow-sm flex flex-col overflow-hidden", mobileView === 'thread' && activeUserId && "hidden lg:flex")}>
            ...
            <div className="flex-1 overflow-y-auto">
              {conversations.length === 0 ? (
                <div className="p-6 text-center">
                  ...
                  <button onClick={() => { setCompose(true); setMobileView('thread'); }} className="mt-3 text-sm font-bold text-peach-500 hover:underline">Start a conversation →</button>
                </div>
              ) : conversations.map(conv => (
                <button
                  key={conv.other_user_id}
                  onClick={() => openThread(conv.other_user_id)}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-1">
                        ...
                        {conv.unread_count > 0 && (
                          <span className="h-5 w-5 rounded-full bg-red-500 text-white text-[10px] font-bold flex items-center justify-center shrink-0">{conv.unread_count}</span>
                        )}
                      </div>
          ...
          {/* Center / Right: Thread or Compose */}
          <div className={cn("lg:col-span-2 bg-white rounded-2xl border border-slate-100 shadow-sm flex flex-col overflow-hidden", mobileView === 'list' && !compose && "hidden lg:flex")}>
            {compose ? (
              ...
              <>
                <div className="px-4 py-3 border-b border-slate-100 flex items-center gap-3">
                  <button onClick={() => { setActiveUserId(null); setMobileView('list'); }} className="p-1 text-slate-400 hover:text-slate-700 lg:hidden"><ArrowLeft size={18} /></button>
                  <div className="h-9 w-9 rounded-full bg-slate-200 text-slate-600 flex items-center justify-center font-bold text-sm shrink-0">
                    {activeContactName?.[0] || '?'}
                  </div>
                  <div>
